...
public class MaquetteController {
    ...
    @PostMapping("/suppMaquette/{id}")
    public String suppMaquette(@PathVariable("id") Long id) {
        Maquette maquette = maquetteRepository.findById(id).orElseThrow();
        maquette.getUe().clear();  // Supprime les associations
        maquetteRepository.save(maquette); // Synchronise avec la base de données
        maquetteRepository.deleteById(id);
        return "redirect:/maquettes/succes" ;
    }
    //les requette sql pour configurer la tables
    /*ALTER TABLE maquette_ue
    ADD CONSTRAINT FK_maquette_ue_ue_id
    FOREIGN KEY (ue_id)
    REFERENCES ue(id)
    ON DELETE CASCADE;
    */
    ...
    public String succes(){
        return "succes"; //la redirect de la page succes apres la suppression de la maquette
    }

    //La methode pour modifier les information dans la maquette
    @PostMapping("/modifier_maquette")
    public String modification_maquette(@ModelAttribute Maquette maquette, RedirectAttributes redirectAttributes) {
        try {
            maquetteService.UpdateMaquette(maquette);
            redirectAttributes.addFlashAttribute("success", "La maquette a été modifiée avec succès.");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("error", "Erreur lors de la modification de la maquette.");
        }
        return "redirect:/maquettes/succes";
    }

    //chemin pour voir maquette et faire des choix
    @GetMapping("/maquettes_pour_choix")
    public String afficherChoix(Model model,
        ...
        if (utilisateur != null) {
            ...
            // Si l'utilisateur a un rôle "ChefDepartement", rediriger vers la page spécifique
            if (utilisateur.getRoles().stream().anyMatch(role -> role.getRole().equals("ChefDepartement"))) {
                return "maquette-choix-chef"; // Rediriger vers la page spécifique pour le Chef de Département
            }
        }

        // Pour les autres utilisateurs, rediriger vers la page par défaut
        return "maquette-choix"; // Page par défaut pour les autres utilisateurs
    }
